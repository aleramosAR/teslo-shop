...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  const { register, handleSubmit, formState:{errors}, getValues, setValue, watch } = useForm({
	...
	const onDeleteTag = (tag: string) => {
		const udpatedTags = getValues('tags').filter(t => t !== tag);
		setValue('tags', udpatedTags, { shouldValidate: true });
	};
...
export const getServerSideProps: GetServerSideProps = async ({ query }) => {
	...
	if(slug === 'new') {
		// Crear un producto (con el parseado evito errores que pueda generar el objeto de mongoose)
		const tempProduct = JSON.parse(JSON.stringify(new Product()));
		// Elimino el ID generado en el nuevo producto
